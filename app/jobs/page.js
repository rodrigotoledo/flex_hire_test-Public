          ...
          <div className="md:w-1/2 p-4 text-center align-center flex flex-col">
            <div className="w-32 h-32 md:w-60 md:h-60 bg-gray-300 rounded-full mx-auto mb-4 shadow-lg">
              <img
                ...
                alt={userData.data.currentUser.name}
              />
            </div>
            <h2 className="text-2xl font-bold">{userData.data.currentUser.name}</h2>
            <p className="text-gray-600">{PHONE_LABEL}: {userData.data.currentUser.phone}</p>
            <p className="text-gray-600">{UPDATED_AT_LABEL}: {userData.updatedAt}</p>
            <p className="text-sky-500">{STATUS_LABEL}: {userData.data.currentUser.status}</p>
          </div>
          <div className="bg-white rounded-md p-4 md:w-1/2">
            <div className="text-left">
              {userData.data.currentUser.allJobs.length === 0 ? (
                <h4 className="text-left">{NO_JOBS_OBTAINED_LABEL}</h4>
              ) : (
                <>
                  {userData.data.currentUser.allJobs.map((job, index) => (
